...
```bash
docker cp input/large_movie_dialogues.txt resourcemanager:/opt/hadoop-3.2.1/share/hadoop/mapreduce/
```
...
```bash
hadoop fs -put large_movie_dialogues.txt /input/movie_scripts/
```
...
```bash
hadoop jar <your-jar-file>.jar com.movie.script.analysis.MovieScriptAnalysis /input/movie_scripts/large_movie_dialogues.txt /output
```
...
2. The input file (`large_movie_dialogues.txt`).
